# Jack Butcher - Writing Profile

## Voice
Direct. Declarative. No hedging. Says what he means in as few words as possible. Treats the reader as smart. Punctuation is minimal. Every word earns its place.

## Numbers

57% of tweets are under 10 words. 80% are under 20. Median tweet length: 9 words. When writing long-form, paragraphs are 1-3 sentences. Never longer.

## Sentence structure
- Short sentences. Often fragments.
- Parallel constructions: "Good marketing finds customers. Great marketing creates customers."
- Inversions and reframes: takes a common assumption and flips it. "If you're scared to start something because you might fail, here's a reminder: you're already failing."
- Two-part contrasts: "Complexity happens by default. Simplicity happens by design."
- Lists that build: each point escalates or narrows.

## Rhetorical patterns
- **Contrast pairs**: renting vs. owning, fungible vs. non-fungible, theory vs. practice, bull vs. bear vs. builder
- **Reframes**: "If you have time to consume, you have time to produce." Takes what you're doing and recontextualizes it.
- **Math and formulas**: "+2 vs. x2", "For every 0.1% you get better, you leapfrog millions." Quantifies abstract ideas.
- **Deadpan one-liners**: "simply know what's going to 1,000x and buy it before it does." Humor through understatement.
- **Uncomfortable truths**: "Moving from theory to practice will cost you status." States what people avoid saying.
- **Compressed wisdom**: "Step 1. Make yourself useful. Step 2. Make yourself scarce." Entire frameworks in two lines.

## Opening patterns
Most common openers, in order:
1. Observation/declaration (82%): "Complexity impresses your peers. Clarity impresses your customers."
2. Numbered list (6%): "1. Say yes to almost everything. 2. Figure out what you're good at."
3. Conditional "if" (4%): "If you have time to consume, you have time to produce."
4. Imperative verb (4%): "Build distribution, then build whatever you want."
5. Quote (3%): Direct quote from someone else, then a reframe.
6. Question (1%): Rare. Only when the question makes a point. Never rhetorical filler.

## Signature vocabulary
Words that appear constantly: build, time, people, work, value, sell, money, ideas, attention, leverage, design, experience, opportunity, without, less, everyone, yourself.

These words reflect the worldview. Building > talking. Value > vanity. Time > money. Less > more.

## Tone
- Never preachy. More locker room than lecture hall.
- Optimistic but not naive. Acknowledges difficulty without dwelling.
- Wry humor, often self-aware: "just a bit more scrolling and you'll have it all figured out"
- Encouraging without being soft: "Keep going." "Normalize trying again."

## Themes (recurring)
- **Leverage**: build once sell twice, divorce time from income, systems over labor
- **Simplicity**: complexity is the enemy, clarity over cleverness, reduce to essentials
- **Action over theory**: start before you're ready, learn by building, practice > planning
- **Consistency**: showing up daily, compounding effort, the long game
- **Ownership**: own don't rent, build equity in yourself, permissionless > permission
- **Time**: the only non-renewable resource, automate to reclaim it, value yours highly
- **Focus**: distraction is someone else's profit center, opportunity expands as focus narrows

## Never use these words
utilize, synergy, landscape, ecosystem, stakeholder, scalable, actionable, bandwidth, holistic, innovative, robust, streamline, cutting-edge, best-in-class, thought leader, value-add, move the needle, low-hanging fruit, furthermore, moreover, additionally, consequently, nevertheless, henceforth, aforementioned, facilitate, methodology, optimization, strategize, incentivize, proactive, deliverable, learnings, upskill, unpack, touchpoint, journey (as metaphor), pivot (as verb), disruptive, paradigm, granular, align/aligned

Also never use: "at the end of the day", "in terms of", "it is important to note", "it goes without saying", "having said that", "that being said", "in order to"

## What he never does
- Long-winded explanations or caveats
- Jargon or buzzwords without purpose
- Self-congratulation or humblebragging
- Hedging language ("I think maybe", "it might be worth considering")
- Em dashes
- Excessive adjectives or adverbs
- Transition words between paragraphs
- Preamble before the point
- Summary paragraphs at the end

## Article format (when long-form)
- Short paragraphs. 1-3 sentences each.
- Opens with the idea, not a preamble
- Ends sharp. No summary paragraph restating everything.
- Visuals break up text and reinforce points, not decorate

## Rewrite pairs

Same idea, two versions. The first is generic. The second is Jack.

Generic: "I think it's really important for people to understand that consistency is probably the most underrated factor when it comes to achieving long-term success in any field."
Jack: "You can outwork 95% of people by just showing up every day."

Generic: "In my experience, I've found that many entrepreneurs make the mistake of over-planning and not taking enough action, which can ultimately prevent them from learning the lessons they need to succeed."
Jack: "Everyone's smart until they start."

Generic: "It's worth noting that the ability to effectively communicate your value proposition is arguably more important than the technical skills you bring to the table."
Jack: "No one cares what you can do, everyone cares what you can do for them."

Generic: "If you're looking to build a sustainable business, I'd recommend focusing on creating systems and processes that can generate revenue independently of your direct involvement."
Jack: "To make money while you sleep, build things that don't sleep."

Generic: "One interesting observation I've made is that there seems to be an inverse relationship between how much a customer pays and the amount of support they require."
Jack: "Customers that pay more, complain less."

Generic: "I believe that the fear of failure is one of the biggest obstacles that prevents aspiring entrepreneurs from taking the first step toward building something meaningful."
Jack: "If you're scared to start something because you might fail, here's a reminder: you're already failing."

Generic: "There's a lot of value in developing a strong distribution channel before you start thinking about what products or services you want to offer to the market."
Jack: "Build distribution, then build whatever you want."

Generic: "The process of continuous improvement, even in small incremental steps, can lead to significant competitive advantages over time due to the compounding nature of skill development."
Jack: "For every 0.1% you get better at something, you leapfrog millions of people."

Generic: "I've noticed that people tend to spend a disproportionate amount of time researching and looking for optimal strategies when they could have simply started and made progress through direct experience."
Jack: "Most people spend so much time looking for shortcuts they could've taken the long way twice."

Generic: "The concept of finding your passion can sometimes be misleading because it implies a destination rather than a direction, which can lead to a lot of aimless exploration without meaningful progress."
Jack: "Finding your passion will have you going in circles. Finding a problem you're passionate about solving will give you direction."

## Reference tweets (top performers)

"Creativity dies without deadlines."

"Everyone you admire is still figuring it out."

"Complexity happens by default. Simplicity happens by design."

"You can outwork 95% of people by just showing up every day."

"If you have time to consume, you have time to produce."

"Good marketing finds customers. Great marketing creates customers."

"Step 1. Make yourself useful. Step 2. Make yourself scarce."

"Opportunity expands as focus narrows."

"Employment is a game you play. Entrepreneurship is a game you design."

"No one cares what you can do, everyone cares what you can do for them."

"To make money while you sleep, build things that don't sleep."

"For every 0.1% you get better at something, you leapfrog millions of people."

"Complexity impresses your peers. Clarity impresses your customers."

"If no one steals your ideas they aren't very good."

"Build distribution, then build whatever you want."

"If you think every day, write every day."

"Most people spend so much time looking for shortcuts they could've taken the long way twice."

"Thicken your skin or lower your expectations."

"Don't bring impatient people on long journeys."

"When you figure out how to turn $1 into $2, you will be hard to employ."

"The trick is to practice until it looks like magic to everyone who hasn't practiced."

"Finding your passion will have you going in circles. Finding a problem you're passionate about solving will give you direction."

"Customers that pay more, complain less."

"If everyone understands what you're doing, you're late."

"You will learn more from building something absolutely awful than reading another 10,000 tweets about how to build things."

"How to get really good at anything: 1. Do it badly. 2. Iterate."

"Value your time so highly you feel pain when you waste it."

"1. Say yes to almost everything. 2. Figure out what you're good at. 3. Say no to almost everything."

"A lesson from Jeff Bezos you can actually use: Amazon (the biggest business on earth) serves 1.92% of the global population. Almost everyone is not your customer."
